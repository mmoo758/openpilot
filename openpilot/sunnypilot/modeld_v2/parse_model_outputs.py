...
class Parser:
  ...
  def is_mhp(self, outs, name, shape):
    """Whether an output is multi-hypothesis. A single-hypothesis head is exactly 2*shape wide
    (mu + std); anything else carries the per-hypothesis weights of an MHP head. Deep-RL combined
    models emit single-hypothesis plan/lead where the older supercombo emitted MHP."""
    if self.check_missing(outs, name):
      return False

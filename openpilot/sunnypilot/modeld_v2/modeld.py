...
class ModelState(ModelStateBase):
  ...
  def get_action_from_model(self, model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action,
    ...
    if 'action' in model_output:
      # deep-RL models emit control directly instead of it being derived from the plan
...
def main(demo=False):
  ...
  while True:
    ...
    frame_delay = DT_MDL  # compensate for time since the frame was captured: current_time - timestamp_eof averages 50ms
    ...
    # deep-RL models take the action horizon as an input and emit control directly
